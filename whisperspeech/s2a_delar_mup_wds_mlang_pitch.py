...
class BinnedEmbedding(nn.Module):
    ...
    def forward(self, x, return_quantized=False):
        shift = 0
#         if self.bins % 2 == 1: # shift by half a bin so it's symmetrical
#             shift = (self.vmax - self.vmin) / self.bins / 2
        # calculate the bin index
        qx = ((x - self.vmin + shift) / (self.vmax - self.vmin) * self.bins).to(torch.long)
...
#         atoks = atoks[:, :, 1:-self.quantizers + 1]
    ...
    def extract_pitch(self, audio):
        ...
        pitches = torch.stack(pitches)#.to(Atoks.device)
#         extra_pad = int(/16000*50 % 2 + .5) # for odd number of pitch samples
...
#             print(semb.shape, self.positional_embeddings.shape)
...
# %% ../nbs/4B. Multi-language semantic to acoustic token modeling-Copy2.ipynb 16
